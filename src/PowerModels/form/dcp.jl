...
function expression_branch_power_ohms_yt_from(
    pm::AbstractDCPModel,
    n::Int,
    f_bus,
    t_bus,
...
function expression_branch_power_ohms_yt_to(
    pm::AbstractDCPModel,
    n::Int,
    f_bus,
    t_bus,
...
function constraint_ohms_y_pst_from(
    pm::AbstractDCPModel,
    n::Int,
    f_bus,
    t_bus,
...
function constraint_ohms_y_pst_to(
    pm::AbstractDCPModel,
    n::Int,
    f_bus,
    t_bus,
...
function constraint_switch_voltage_on_off(
    pm::AbstractDCPModel,
    n::Int,
    i,
    f_bus,
...
function expression_bus_voltage(
    pm::AbstractPowerModel,
    n::Int,
    i,
    am::AdmittanceMatrixInverse,
)
...
function variable_branch_power_real(
    ...
    p =
        var(pm, nw)[:p] = JuMP.@variable(pm.model,
    ...
    if bounded
        flow_lb, flow_ub =
            ref_calc_branch_flow_bounds(ref(pm, nw, :branch), ref(pm, nw, :bus))
    ...
    p_expr =
        Dict{Any, Any}(((l, i, j), p[(l, i, j)]) for (l, i, j) in ref(pm, nw, :arcs_from))
    p_expr = merge(
        p_expr,
        Dict(((l, j, i), -1.0 * p[(l, i, j)]) for (l, i, j) in ref(pm, nw, :arcs_from)),
    )
    var(pm, nw)[:p] = p_expr

    report && sol_component_value_edge(
        pm,
        nw,
        :branch,
        :pf,
...
function variable_ne_branch_power_real(
    pm::AbstractAPLossLessModels;
    nw::Int = nw_id_default,
    bounded::Bool = true,
    report::Bool = true,
    ...
    # this explicit type erasure is necessary
    p_ne_expr = Dict{Any, Any}([
        ((l, i, j), 1.0 * var(pm, nw, :p_ne, (l, i, j))) for
        (l, i, j) in ref(pm, nw, :ne_arcs_from)
    ])
    p_ne_expr = merge(
        p_ne_expr,
        Dict(
            ((l, j, i), -1.0 * var(pm, nw, :p_ne, (l, i, j))) for
            (l, i, j) in ref(pm, nw, :ne_arcs_from)
        ),
    )
    ...
    report && sol_component_value_edge(
        ...
        :pf,
        :pt,
        ref(pm, nw, :ne_arcs_from),
        ref(pm, nw, :ne_arcs_to),
        p_ne_expr,
    )
end

function constraint_network_power_balance(
    pm::AbstractAPLossLessModels,
    n::Int,
    i,
    comp_gen_ids,
    ...
    JuMP.@constraint(
        ...
        sum(pd for (i, pd) in values(comp_pd)) +
        sum(gs * 1.0^2 for (i, gs) in values(comp_gs))
    )
    # omit reactive constraint
end
...
"nothing to do, this model is symetric"
function constraint_thermal_limit_to_on_off(
    pm::AbstractAPLossLessModels,
    n::Int,
    i,
    t_idx,
...
"nothing to do, this model is symetric"
function constraint_ne_thermal_limit_to(
    pm::AbstractAPLossLessModels,
    n::Int,
    i,
    t_idx,
...
function constraint_storage_on_off(
    pm::AbstractAPLossLessModels,
    n::Int,
    i,
    pmin,
...
function constraint_storage_losses(
    pm::AbstractAPLossLessModels,
    n::Int,
    i,
    bus,
...
"nothing to do, no voltage angle variables"
function constraint_voltage_angle_difference(
    pm::AbstractNFAModel,
    n::Int,
    f_idx,
    angmin,
...
function constraint_ohms_yt_to_on_off(
    ...
    JuMP.@constraint(
        pm.model,
        p_fr + p_to >= r * ((-b * (va_fr - va_to))^2 - (-b * (t_m))^2 * (1 - z))
    )

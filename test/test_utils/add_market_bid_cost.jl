...
`linear = true` wraps each stepped scalar in `IS.LinearFunctionData(value, 0.0)` instead of
leaving it as a bare `Number` — kept for callers (e.g. `test_mbc_parameter_population.jl`)
that still build a `TimeSeriesLinearCurve`-keyed series from a plain `Float64` base value
rather than passing an `IS.LinearFunctionData` `ini_val` directly.
"""

...
function moi_tests(
    ...
    JuMPmodel = POM.get_jump_model(model)
...
function psi_constraint_test(
    model::DecisionModel,
    constraint_keys::Vector{<:POM.ConstraintKey},
)
    constraints = POM.get_constraints(model)
...
function psi_aux_variable_test(
    model::DecisionModel,
    constraint_keys::Vector{<:POM.AuxVarKey},
)
    op_container = POM.get_optimization_container(model)
    vars = POM.get_aux_variables(op_container)
...
function psi_checkbinvar_test(
    model::DecisionModel,
    bin_variable_keys::Vector{<:POM.VariableKey},
)
    container = POM.get_optimization_container(model)
    for variable in bin_variable_keys
        for v in POM.get_variable(container, variable)
...
function psi_checkobjfun_test(model::DecisionModel, exp_type)
    model = POM.get_jump_model(model)
...
function moi_lbvalue_test(
    model::DecisionModel,
    con_key::POM.ConstraintKey,
    value::Number,
)
    for con in POM.get_constraints(model)[con_key]
...
function psi_ptdf_lmps(res::OptimizationProblemResults, ptdf)
    cp_duals =
        read_dual(res, POM.ConstraintKey(CopperPlateBalanceConstraint, PSY.System))
    λ = Matrix{Float64}(cp_duals[:, propertynames(cp_duals) .!= :DateTime])

    flow_duals = read_dual(res, POM.ConstraintKey(NetworkFlowConstraint, PSY.Line))
...
function check_variable_unbounded(
    ...
    ::Type{T},
    ::Type{U},
) where {T <: POM.VariableType, U <: PSY.Component}
    return check_variable_unbounded(model::DecisionModel, POM.VariableKey(T, U))
end

function check_variable_unbounded(model::DecisionModel, var_key::POM.VariableKey)
    psi_cont = POM.get_optimization_container(model)
    variable = POM.get_variable(psi_cont, var_key)
    for var in variable
...
function check_variable_bounded(
    ...
    return check_variable_bounded(model, POM.VariableKey(T, U))
end

function check_variable_bounded(model::DecisionModel, var_key::POM.VariableKey)
    psi_cont = POM.get_optimization_container(model)
    variable = POM.get_variable(psi_cont, var_key)
    for var in variable
        if !JuMP.has_lower_bound(var) || !JuMP.has_upper_bound(var)
...
function check_flow_variable_values(
    ...
    psi_cont = POM.get_optimization_container(model)
    variable = POM.get_variable(psi_cont, T(), U)
    for var in variable[device_name, :]
        if !(POM.jump_value(var) <= (limit + 1e-2))
            @error "$device_name out of bounds $(POM.jump_value(var))"
            return false
        end
    end
    return true
...
function check_flow_variable_values(
    ...
    device_name::String,
    limit::Float64,
) where {T <: POM.FlowActivePowerVariable, U <: PSY.Component}
    psi_cont = POM.get_optimization_container(model)
    template = model.template
    device_model = POM.get_model(template, U)
    dev_formulation = POM.get_formulation(device_model)
    net_formulation = POM.get_network_formulation(template)
    if dev_formulation <: Union{POM.StaticBranch, POM.StaticBranchUnbounded} &&
       net_formulation <: POM.PTDFPowerModel
        variable = POM.get_expression(psi_cont, POM.PTDFBranchFlow(), U)
    else
        variable = POM.get_variable(psi_cont, T(), U)
    end
    for var in variable[device_name, :]
...
function check_flow_variable_values(
    ...
    variable = POM.get_variable(psi_cont, T(), U)
    for var in variable[device_name, :]
        if !(POM.jump_value(var) <= (limit_max + 1e-2)) ||
           !(POM.jump_value(var) >= (limit_min - 1e-2))
            return false
        end
    end
    return true
...
function check_flow_variable_values(
    ...
    limit_min::Float64,
    limit_max::Float64,
) where {T <: POM.VariableType, U <: POM.VariableType, V <: PSY.Component}
    psi_cont = POM.get_optimization_container(model)
    time_steps = POM.get_time_steps(psi_cont)
    pvariable = POM.get_variable(psi_cont, T(), V)
    qvariable = POM.get_variable(psi_cont, U(), V)
    for t in time_steps
        fp = POM.jump_value(pvariable[device_name, t])
        fq = POM.jump_value(qvariable[device_name, t])
        flow = sqrt((fp)^2 + (fq)^2)
...
function POM.jump_value(int::Int)
...
function check_duration_on_initial_conditions_values(
    ...
    initial_conditions_data =
        POM.get_initial_conditions_data(POM.get_optimization_container(model))
    duration_on_data = POM.get_initial_condition(
    ...
    for ic in duration_on_data
        ...
        on_var = POM.get_initial_condition_value(initial_conditions_data, OnVariable(), T)[
            name,
            1,
        ]
        duration_on = POM.jump_value(POM.get_value(ic))
...
function check_duration_off_initial_conditions_values(
    ...
    initial_conditions_data =
        POM.get_initial_conditions_data(POM.get_optimization_container(model))
    duration_off_data = POM.get_initial_condition(
        POM.get_optimization_container(model),
    ...
    for ic in duration_off_data
        ...
        duration_off = POM.jump_value(POM.get_value(ic))
...
function check_energy_initial_conditions_values(model, ::Type{T}) where {T <: PSY.Component}
    ic_data = POM.get_initial_condition(
        POM.get_optimization_container(model),
        InitialEnergyLevel(),
        T,
    )
    for ic in ic_data
        d = ic.component
        name = PSY.get_name(ic.component)
        e_value = POM.jump_value(POM.get_value(ic))
...
function check_status_initial_conditions_values(model, ::Type{T}) where {T <: PSY.Component}
    initial_conditions =
        POM.get_initial_condition(POM.get_optimization_container(model), DeviceStatus(), T)
    initial_conditions_data =
        POM.get_initial_conditions_data(POM.get_optimization_container(model))
    for ic in initial_conditions
        name = PSY.get_name(ic.component)
        status = POM.get_initial_condition_value(initial_conditions_data, OnVariable(), T)[
            name,
            1,
        ]
        @test POM.jump_value(POM.get_value(ic)) == status
    end
end
...
function check_active_power_initial_condition_values(
    ...
    initial_conditions =
        POM.get_initial_condition(POM.get_optimization_container(model), DevicePower(), T)
    initial_conditions_data =
        POM.get_initial_conditions_data(POM.get_optimization_container(model))
    for ic in initial_conditions
        name = PSY.get_name(ic.component)
        power = POM.get_initial_condition_value(
            initial_conditions_data,
            ActivePowerVariable(),
            T,
        )[
        ...
        @test POM.jump_value(POM.get_value(ic)) == power
    end
end
...
function check_active_power_abovemin_initial_condition_values(
    ...
    initial_conditions = POM.get_initial_condition(
        POM.get_optimization_container(model),
...
function check_initialization_variable_count(
    ...
    ::S,
    ::Type{T},
) where {S <: POM.VariableType, T <: PSY.Component}
    container = POM.get_optimization_container(model)
    initial_conditions_data = POM.get_initial_conditions_data(container)
    no_component = length(PSY.get_components(PSY.get_available, T, model.sys))
    variable = POM.get_initial_condition_value(initial_conditions_data, S(), T)
    rows, cols = size(variable)
    @test rows * cols == no_component * POM.INITIALIZATION_PROBLEM_HORIZON_COUNT
end
...
function check_variable_count(
    ...
    no_component = length(PSY.get_components(PSY.get_available, T, model.sys))
    time_steps = POM.get_time_steps(POM.get_optimization_container(model))[end]
    variable = POM.get_variable(POM.get_optimization_container(model), S(), T)
...
function check_constraint_count(
    ...
    no_component = length(PSY.get_components(filter_func, T, model.sys))
    time_steps = POM.get_time_steps(POM.get_optimization_container(model))[end]
    constraint = POM.get_constraint(POM.get_optimization_container(model), S(), T, meta)
    @test length(constraint) == no_component * time_steps
end
...
function check_constraint_count(
    ...
    device_name_set =
        PSY.get_name.(
            POM._get_ramp_constraint_devices(
                container,
...
function check_constraint_count(
    ...
    container = POM.get_optimization_container(model)
    resolution = POM.get_resolution(container)
